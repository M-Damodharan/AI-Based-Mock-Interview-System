...
def userhome():
    ...
    if request.method=='POST':
        language=request.form['program']

        mycursor.execute("SELECT * FROM vh_apti_question order by rand() limit 0,5")
        ...
        prid=0
        mycursor.execute("SELECT * FROM vh_program where language=%s order by rand()",(language,))
        dd2 = mycursor.fetchall()
        for dd3 in dd2:
            prid=dd3[0]

        mycursor.execute("update vh_candidate set question=%s,program=%s where username=%s",(ques,prid,uname))
        mydb.commit()
        ...
        sql = "INSERT INTO vh_profile_matched(id,vacancy_id,username,interview_status,interview_date,interview_time,apti,hr_id,company) VALUES (%s,%s,%s,%s, %s, %s,%s,%s,%s)"
        val = (maxid,'0',uname,'1',rdate,rtime,'',prid,language)
        pid=str(maxid)
        mycursor.execute(sql, val)
        mydb.commit()
        msg="ok"
...
@app.route('/test_details',methods=['POST','GET'])
def test_details():
    msg=""
    uname=""
    edata=[]
    ...
    mycursor.execute("SELECT * FROM vh_profile_matched p,vh_candidate c where p.username=c.username && p.username=%s",(uname,))
    mdata = mycursor.fetchall()


    return render_template('test_details.html',msg=msg,data=data,mdata=mdata)

@app.route('/view_test',methods=['POST','GET'])
def view_test():
    msg=""
    uname=""
    adata=[]
    ...
    mycursor.execute("SELECT * FROM vh_temp t,vh_apti_question a where t.pid=%s && t.qid=a.id",(pid, ))
    adata = mycursor.fetchall()
    
    
    mycursor.execute("SELECT * FROM vh_profile_matched p,vh_candidate c where p.username=c.username && p.username=%s && p.id=%s",(uname,pid))
    ...
    mycursor.execute("SELECT * FROM vh_temp2 t,vh_interview_question q where t.qid=q.id && t.pid=%s",(pid, ))
    vdata = mycursor.fetchall()

    return render_template('view_test.html',msg=msg,data=data,mdata=mdata,adata=adata,prm=prm,vdata=vdata,s1=s1,s2=s2,s3=s3)
...
def admin_test():
    ...
    mycursor = mydb.cursor()
    mycursor.execute("SELECT * FROM vh_candidate where id=%s",(aid, ))
    data = mycursor.fetchone()
    uname=data[6]
    ...
    return render_template('admin_test.html',msg=msg,data=data,mdata=mdata,aid=aid,pid=pid)

@app.route('/admin_testview',methods=['POST','GET'])
def admin_testview():
    msg=""
    uname=""
    adata=[]
    ...
    return render_template('admin_testview.html',msg=msg,data=data,mdata=mdata,adata=adata,prm=prm,vdata=vdata,s1=s1,s2=s2,s3=s3,aid=aid,pid=pid)
...
def upload_resume():
    ...
    return render_template('upload_resume.html',data=data,msg=msg)
...
def upload_photo():
    ...
    return render_template('upload_photo.html',msg=msg,data=data)
...
def add_test():
    ...
    mycursor = mydb.cursor()
    mycursor.execute("SELECT * FROM vh_interview_question")
    data = mycursor.fetchall()
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT max(id)+1 FROM vh_interview_question")
        maxid = mycursor.fetchone()[0]
        if maxid is None:
            maxid=1
    
        
        sql = "INSERT INTO vh_interview_question(id,question,answer) VALUES (%s, %s, %s)"
        val = (maxid,question,answer)
    ...
    if act=="del":
        did=request.args.get("did")
        mycursor.execute("delete from vh_interview_question where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('add_test'))
        
        
    return render_template('add_test.html',msg=msg,data=data)
...
def test_program1():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("update vh_profile_matched set program_score=0,dress_code=%s where id=%s",(fname,pid))
        mydb.commit()
        ###                
        ff=open(path+"/program/"+fname,"w")
        ff.write(ccode)
...
def read_file():
    ...
    filepath = path+"/program/"+filename
